...
describe('token', () => {
  test('instantiates correctly', () => {
    ...
    const token = new Token(ChainId.GOERLI, address, 18, 'TST', 'Test token')
  ...
  describe('equals', () => {
    ...
    test('returns true if tokens have the same chain id and address', () => {
      const address = Wallet.createRandom().address
      const token1 = new Token(ChainId.GOERLI, address, 18, 'TST1', 'Test token 1')
      const token2 = new Token(ChainId.GOERLI, address, 18, 'TST2', 'Test token 2')
